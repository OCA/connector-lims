...
class ProductTemplate(models.Model):
    ...
    def _prepare_lims_sample_vals(self, sale_order, analytes, sale_origin=None):
        ...
        return {
            ...
            "analysis_ids": [
                Command.create({"analyte_id": analyte.id}) for analyte in analytes
            ],
        }

    def _create_lims_sample(self, sale_order, analytes, sale_origin=None):
        self.ensure_one()
        if self.lims_tracking == "no" or not self.lims_sample_type_id or not analytes:
            return self.env["lims.sample"]
        return self.env["lims.sample"].create(
            self._prepare_lims_sample_vals(
                sale_order, analytes, sale_origin=sale_origin
            )
        )
