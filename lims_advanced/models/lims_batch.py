...
class LIMSBatch(models.Model):
    ...
    def action_complete(self):
        batch_completed_stage = self.env.ref(
            "lims_advanced.lims_stage_batch_completed", raise_if_not_found=False
        )
        for batch in self:
            incomplete_analyses = batch.analysis_ids.filtered(lambda a: not a.is_closed)
            if incomplete_analyses:
                raise ValidationError(
                    _(
                        "You cannot complete this batch because some associated "
                        "analyses are not yet verified:\n %s"
                    )
                    % ", ".join(incomplete_analyses.mapped("identifier"))
                )
    ...
    def action_cancel(self):
        return self.write(
            {"stage_id": self.env.ref("lims_advanced.lims_stage_batch_cancelled").id}
        )
    ...
    def _onchange_analysis_ids(self):
        if self.operator_id or self.scheduled_date or self.instrument_id:
            ...
            self.analysis_ids.write(values)
